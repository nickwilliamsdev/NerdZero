...
Version 24 memetic/Lamarckian geometry-enriched evolved PyTorch-NEAT CPPN transport + finite-horizon MuZero program-search follow-up to the supervised transport algebra of a geometry-generated operator model inspired by the architecture
we discussed:
...
def evolve_transport_cppn(model: TinyReasoner, generations: int = 100, pop_size: int = 256,
                          ...
                          workers: int = 12, inner_steps: int = 12, inner_lr: float = 1e-2):
    ...
    def score_one(item, neat_config):
        gid, genome = item
        try:
            # Memetic inner loop: topology comes from NEAT; weights/biases/responses
            # receive a short backprop optimization, then are written back to genes.
            ...
            # Re-materialize using the actual Uber PyTorch-NEAT implementation so
            # fitness measures exactly what descendants/downstream code will use.
    ...
    def eval_genomes(genomes, neat_config):
        ...
        for (gid, fitness, ce_v, acc_v, ent_v, c2_ce_v, c2_acc_v, c3_ce_v,
             c3_acc_v, loss0_v, lossT_v, learn_v, auc_v) in results:
            genome = by_id[gid]
