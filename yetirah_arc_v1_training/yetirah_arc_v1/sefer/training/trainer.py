...
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.algebra.rollout import apply_program_actions, apply_selected_actions, differentiable_rollout, operator_separation_loss
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.controllers.reasoner import TinyReasoner
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.evaluation.planning import evaluate_muzero_program_search, evaluate_root_search_diagnostics
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.evaluation.primitives import evaluate_held_out, evaluate_operator_rollout
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.evaluation.programs import evaluate_exact_variable_program_search, evaluate_greedy_program_accuracy
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.evolution.evolve_transport import antithetic_es_step_, evolve_transport_cppn, load_evolved_transport_cppn
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.planning.bellman import exact_horizon_policy_target, exact_horizon_value_target, soft_policy_cross_entropy
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.planning.puct import SearchConfig, goal_puct_search, root_action_diagnostics
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.tasks.synthetic_algebra import SyntheticTaskBatch
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.training.losses import transport_supervision_loss
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.training.phases import freeze_for_program_phase, set_query_codec_trainable, prepare_fast_program_phase
from yetirah_arc_v1_training.yetirah_arc_v1.sefer.utils import seed_all
...
def train_smoke_test(
    ...
    # Reuse the expensive evolved geometry whenever possible. A fresh evolution
    # still runs automatically if the winner file does not exist.
    if reuse_neat_winner and os.path.exists(neat_winner_path):
        print(f"loading evolved transport CPPN: {neat_winner_path}")
        load_evolved_transport_cppn(model, neat_winner_path)
    else:
        print(f"evolving shared transport CPPN: generations={neat_generations} population={neat_population}")
        evolve_transport_cppn(
            model, generations=neat_generations, pop_size=neat_population,
            seed=neat_seed, workers=neat_workers, inner_steps=neat_inner_steps,
            inner_lr=neat_inner_lr, save_path=neat_winner_path,
        )

    start_step = 1
    if resume_from_algebra and os.path.exists(algebra_checkpoint_path):
        payload = torch.load(algebra_checkpoint_path, map_location=torch_device)
        model.load_state_dict(payload["model_state_dict"], strict=False)
        set_query_codec_trainable(model, False)
        prepare_fast_program_phase(model, materialize_frozen_operators)
        start_step = algebra_steps + 1
        print(f"resuming from frozen algebra checkpoint: {algebra_checkpoint_path}")
    ...
    for step in range(start_step, steps + 1):
        ...
        if step == algebra_steps + 1 and start_step == 1:
            # Save the validated algebra separately so future controller/search
            # experiments can skip warmup + algebra as well as NEAT evolution.
            torch.save({"model_state_dict": model.state_dict(), "step": algebra_steps}, algebra_checkpoint_path)
            print(f"saved frozen-algebra checkpoint: {algebra_checkpoint_path}")
            prepare_fast_program_phase(model, materialize_frozen_operators)
            # Rebuild the optimizer over only trainable program parameters.
            optimizer = torch.optim.AdamW(
                [p for p in model.parameters() if p.requires_grad],
                lr=3e-4, weight_decay=1e-4,
            )
            if model.core.has_materialized_operator_bank:
                print(f"materialized operator bank: {tuple(model.core._transport_bank.shape)}")
    ...
    checkpoint_path = posttrain_checkpoint_path
