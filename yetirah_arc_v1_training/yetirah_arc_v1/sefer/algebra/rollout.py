...
def differentiable_rollout(
    ...
    for _ in range(steps):
        ...
        stack = model.core.apply_all_operators(H)
...
def apply_selected_actions(model, H, actions, adjacency=None):
    # The core owns the optimized batched execution path. adjacency is retained
    # only for backward-compatible call sites.
    return model.core.apply_operator_batch(H, actions)
...
def operator_separation_loss(
    ...
    Hs = H[:max_batch]
    A = model.core.adjacency()
    states = model.core.apply_all_operators(Hs)
