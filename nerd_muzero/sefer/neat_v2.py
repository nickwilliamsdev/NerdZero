...
Version 23 geometry-enriched evolved PyTorch-NEAT CPPN transport + finite-horizon MuZero program-search follow-up to the supervised transport algebra of a geometry-generated operator model inspired by the architecture
we discussed:

5D hypercube substrate (32 vertices)
    -> legacy differentiable adjacency control + evolved PyTorch-NEAT transport CPPN
...
# ---------------------------------------------------------------------------
# Real NEAT-Python + Uber PyTorch-NEAT integration
# ---------------------------------------------------------------------------
...
def load_evolved_transport_cppn(model: TinyReasoner, winner_path: str = "yetirah_v23_neat_winner.pkl"):
...
def evolve_transport_cppn(model: TinyReasoner, generations: int = 100, pop_size: int = 100,
                          seed: int = 0, save_path: str = "yetirah_v23_neat_winner.pkl",
                          workers: int = 12):
    ...
    def score_one(item, neat_config):
        ...
        try:
            ...
            # Do not allow evolution to sacrifice one primitive while averaging
            # highly across the others. The bottleneck term creates strong
            # pressure for a genuinely shared four-law transport generator.
            fitness = (
...
def train_smoke_test(
    ...
    print(f"mode=v23 geometry-enriched DGX-scaled REAL PyTorch-NEAT shared transport CPPN + bottleneck/composition-aware fitness + functional-equivalence + transposition MuZero warmup({warmup_steps}) -> supervised transport algebra -> frozen-algebra variable-length (1..4) goal-conditioned program inference")
    ...
    checkpoint_path = "yetirah_v23_posttrain.pt"
